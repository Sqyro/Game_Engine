...
in vec2 TexCoord; //Variable vom Vertexshader, damit wir wissen wo auf der Textur gezeichnet wird
...
#define MAX_LIGHTS 64 //Maximalanzahl an Lichtern, weil OpenGl keine Dynamischen Arrays kann (Muss es ja die Länge beim compilen wissen)

uniform int activeLights; //Anzahl aktiver Lichter
uniform vec2 lightPositions[MAX_LIGHTS]; //Positionen der Lichter
uniform vec3 lightColors[MAX_LIGHTS]; //Farben der Lichter
